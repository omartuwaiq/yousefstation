...
export default class Restaurantbill extends Component {
  ...
  deleteSpecialist(RestaurantsId) {
    axios
      .delete(
        `https://yousefbackend.herokuapp.com/api/menu/delete/${RestaurantsId}`
      )
      .then((response) => {
        const RestaurantsList = this.state.RestaurantsList.filter(
          (item) => item.RestaurantsId !== RestaurantsId
        );
              ...
              {this.state.RestaurantsList.map((item) => (
                <tr key={item.RestaurantsId}>
                  ...
                  <td>
                    <button
                      onClick={(e) =>
                        this.deleteSpecialist(item.RestaurantsId, e)
                      }
                    >
                      delete
                    </button>
